...
namespace json {
    ...
    inline JSON Array() {   // inline
        return std::move(JSON::Make(JSON::Class::Array));
    }

    template <typename... T>
    inline JSON Array(T... args) { // inline
        JSON arr = JSON::Make(JSON::Class::Array);
        arr.append(args...);
        return std::move(arr);
    }

    inline JSON Object() {  // inline
        return std::move(JSON::Make(JSON::Class::Object));
    }

    inline std::ostream& operator<<(std::ostream& os, const JSON& json) {  // inline
        os << json.dump();
        return os;
    }
    ...
    inline JSON JSON::Load(const string& str) { // inline
        size_t offset = 0;
